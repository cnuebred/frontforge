...
By default, FrontForge parses basic Markdown syntax inside your Widget content and converts it safely to HTML. The internal text_to_markdown engine automatically sanitizes inputs to prevent Cross-Site Scripting (XSS).
...
Instead of adding and removing widgets from the DOM constantly (which is computationally expensive), FrontForge allows you to mount logic to decide whether a widget should be visible or hidden (via CSS display: none) using the show_when() lifecycle method.  
```ts
...
adminPanel.show_when(() => uiState.target.userLoggedIn);

uiState.set_setter_callback(() => {  
  adminPanel.render(); // Re-evaluates the show_when condition  
});
...
Need to create multiple similar elements without rewriting the definition? Use .clone(). By default, it duplicates the widget, its styles, its attributes, and optionally re-binds its event listeners (with_events: true).  
```ts
...
async function compileApp() {  
  ...
  await bundle.script("./src/main.ts", { API_URL: "'[https://api.myapp.com](https://api.myapp.com)'" }, "1.0.0");
...
```ts
import { Widget, ContainerWidget, Flex, Pocket, flex_direction_e } from "@cnuebred/frontforge";
...
const applicationLayout = Flex([  
  appTitle,  
  Flex([taskInput, submitBtn], { column_gap: "10px", width: "100%" }),  
  listContainer  
], {  
  direction: flex_direction_e.column,  
  row_gap: "20px",  
